...
## `pilot-schedule-w02.json` — REMOVED (Pf39c2-social-pilot-02a D01, 2026-08-27)

This fixture existed only to exercise `cli.test.ts`'s end-to-end Objection
render test (`render — end-to-end: The Objection`, F08) against a schedule
with an Objection slot in it — week 1 has none. Question, Objection and
Still were deleted outright (see that plan's "Deprecation — one Wall a day"
section): the channel is one Wall a day, drawn from the Wall pool, nothing
else. With no Objection render path left to exercise, this fixture (and the
test that read it) were deleted along with it.

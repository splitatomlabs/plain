...
// --- Real, committed week-2 schedule (generated by
// `npx tsx scripts/generate-schedule.ts --week 2 --seed 42 --objection-weight 20
// --skip-review-check` — F08: week 1 contains no Objection slot at all (weight 1,
// capped at one per week, and week 1's draw gave it none), so week 2 is generated,
// using the CLI's own documented --skip-review-check escape hatch rather than
// faking a review note, with an inflated --objection-weight to reliably land the
// format's one allowed slot) ---
const WEEK_2_SCHEDULE = JSON.parse(
	readFileSync(path.join(scheduleDir, scheduleFileName(2)), 'utf-8')
) as WeekSchedule;
...
// the `objection-*.mp4` naming path had never executed together. Week 2
// (`content/social/pilot-schedule-w02.json`) supplies a real Objection slot
// — see the WEEK_2_SCHEDULE comment above for how it was generated.
// ---------------------------------------------------------------------------

...
//
// Pf39c2-social-pilot-02a D02: this file was left un-regenerated on purpose
// (D04's job) — it still carries its pre-D02 two-slot-per-day shape
// (`slot`/`read_through`/`read_through_counter` fields, and `still`/
// `question` formats on some days). `resolveDay` (day-only lookup) picks the
// FIRST slot entry for a given day, which for every day this file's tests
// actually use (1 and 6) is already a real Wall slot under the current
// committed data — this suite deliberately only exercises those two days,
// so it passes against both the stale file today and the D04-regenerated,
// truly-single-slot file later with no changes needed.
...
describe('render — end-to-end: a real MP4, IG feed still, and metadata sidecar', () => {
	...
	it(
		...
		async () => {
			// Day 6 of the REAL committed week-1 schedule (meditations-02-006) —
			// a real Wall slot.
			const slot = resolveDay(WEEK_1_SCHEDULE, 6);
			if (slot.content.format !== 'wall') throw new Error('expected a wall slot');

...
// Pf39c2-social-pilot-02a D01: this used to also carry a Question-format
// sibling sweep (social pilot 02a REVIEW R03) proving the chapter block
// cleared the travel floor for every non-excluded `content/social/premises/
// question.json` entry. Question was deleted outright (the channel is one
// Wall a day, drawn from the Wall pool, nothing else) along with that pool
// file, so this file's coverage is Wall-only now — see the R02 sweep above.

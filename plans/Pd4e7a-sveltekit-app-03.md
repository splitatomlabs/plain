...
- [ ] T09: Integration test — Run `npm run build` and `npm run preview`. Navigate to a real card (e.g., `/meditations/book-01/1`). Verify: card text renders in Literata, "Show original" toggle works, tag pills link to tag pages, prev/next navigation works (click, keyboard arrows), progress bar shows correct position, page works at 375px mobile and 1280px desktop, dark mode renders correctly, OG meta tags present in page source. Navigate across chapter boundaries.

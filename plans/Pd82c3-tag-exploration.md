...
- [x] T09: Tag index page — progress badges — On `/tags` page, load tag progress from store on mount. Pass cards-read count to each tag card. Display badge on each tag showing progress. Files: `web/src/routes/tags/+page.svelte`
- [x] T10: Home page — browse by theme section — Add a "Browse by theme" section after all author sections on the home page. Render all 8 tags as TagPills with progress badges. Section heading: "Browse by theme" in UI font. Anchor ID `#themes` for the hero link. Files: `web/src/routes/+page.svelte`

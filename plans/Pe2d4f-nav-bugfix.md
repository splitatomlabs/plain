...
## Tasks
- [ ] T01: Reproduce symptom 1 in a failing Playwright test — on `/`, open menu, click "Home" link, assert: drawer hidden, `main#main-content` has no `inert` attr, `body` has no inline `overflow` style, a button inside `<main>` is clickable. Add to `web/tests/e2e/navigation.spec.js` under a new `describe('Main menu self-healing')`.
- [ ] T02: Reproduce symptom 2 in a failing Playwright test — from `/`, navigate to `/enchiridion` via a card link, then back via `page.goBack()` AND separately via the menu Home link; assert the home page's "Continue Reading" / author rings update (or at least that a visible button on `/` responds to click, and `main` is not inert).
- [ ] T03: Add a WebKit-specific variant of T01/T02 by parameterising over `['chromium', 'webkit']` or confirming `playwright.config` already runs both projects. Update config if needed.
- [ ] T04: Run the new tests; confirm they FAIL against current `main` (document the failure output inline in the plan as a comment on the task).
- [ ] T05: Refactor `MainMenu.svelte` — delete `setBackgroundInert`, delete `INERT_SELECTORS`, delete the body-overflow mutation and the `onDestroy` cleanup of same. Expose `open` via either a `$lib/stores/menuState.js` store or a prop callback. Keep focus management, Escape handling, and tab trap.
- [ ] T06: Update `+layout.svelte` — import the menu-open signal; apply `inert={menuOpen}` to `<main id="main-content">` and `<footer class="site-footer">` (and skip link) as a reactive attribute binding; toggle a `menu-open` class on `document.body` via a `$effect` that has an explicit return cleanup so it cannot leak.
- [ ] T07: Add CSS for `body.menu-open { overflow: hidden; }` in `web/src/app.css` (replacing inline style mutation).

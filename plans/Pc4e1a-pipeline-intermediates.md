...
## Tasks
- [x] T01: Migrate existing cache files — copy `.pipeline-cache/{book}-refine.json` to `pipeline/{book}/refine.json` (and translate), skip test artifact files (enchiridion, meditations with tiny sizes). Verify file integrity after copy.
- [x] T02: Update `scripts/lib/cache.ts` — change `CACHE_DIR` to `path.resolve("pipeline")`, update `refinePath`/`translatePath` to use `pipeline/{bookSlug}/refine.json` structure, update doc comment
- [x] T03: Update `scripts/lib/__tests__/cache.test.ts` — update comment references from `.pipeline-cache` to `pipeline`
- [x] T04: Update `.gitignore` — remove `.pipeline-cache/` line
- [x] T05: Remove old `.pipeline-cache/` directory
- [x] T06: Run tests — `npm test` must pass (174 + 36 tests)
- [x] T07: Run `--book shortness-of-life` (cached) to verify reads work from new path, then `--book enchiridion --fresh` to verify writes work

...
- [ ] T06: Update CLAUDE.md — update doc refs to `docs/` prefix; add "Local Development" section (npm run dev, --host for mobile); add "Deploy" section (Vercel auto-deploy from main, preview deploys on PRs); update Screenshots section to not use `cd`
- [ ] T07: Update README.md — update Documentation section refs to `docs/`; add "Local Development" section with dev server and mobile testing via `--host`; add "Deploy" section explaining Vercel setup; update Content Pipeline section with new directory names; update Testing section with more detail on test structure and e2e

...
- [ ] T03: Build favorites store — Extend progress.js or create alongside it. localStorage key `plain-favorites` (string array of card IDs). Exports: `toggleFavorite(cardId)`, `isFavorite(cardId)`, `getFavorites()`. Add heart/favorite button to Card.svelte — icon toggles filled/outline, `aria-label` announces state.

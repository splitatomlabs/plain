...
- [ ] T06: Build ProgressRing component — SVG circle with stroke-dasharray for progress arc. Props: percentage, size, accent color (author-specific), label. Includes `role="img"` with `aria-label` ("Meditations: 47 of 120 cards read, 39%"). Used on returning-reader home page for per-author progress overview. Animated fill on mount (unless reduced motion). Three sizes: small (for book cards), medium (for home page author rings).

...
- [ ] T04: Wire progress to card navigation — In CardNav.svelte, when user navigates to next card: call `markCardRead(bookSlug, currentCardId)`. Do NOT mark on page load (prevents shared-link inflation). Update `last_card` and `last_read_at` timestamps. After marking, check if milestone threshold crossed (25/50/75/100%) — if so, trigger milestone event.
